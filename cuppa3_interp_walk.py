...
#########################################################################
def arrdec_stmt(node):

    (ARRDEC, name, arr_init) = node
    assert_match(ARRDEC, 'arrdec')

    array = '[' + str(walk(arr_init)) + ']'
    state.symbol_table.declare_array(name, array)
 

 #########################################################################
def append_exp(node):

    (APPEND, name, exp) = node
    assert_match(APPEND, 'append')
    
    append_val = walk(exp);
    (arr_name, arr) = state.symbol_table.lookup_sym(name)
    new_arr = (arr[:(len(arr)-1)] + ', ' + str(append_val) + ']')
    state.symbol_table.update_sym(name, ('array', new_arr))

 #########################################################################
def prepend_exp(node):

    (PREPEND, name, exp) = node
    assert_match(PREPEND, 'prepend')
    
    prepend_val = walk(exp);
    (arr_name, arr) = state.symbol_table.lookup_sym(name)
    new_arr = ('[' + str(prepend_val) + ', ' + arr[1:])
    state.symbol_table.update_sym(name, ('array', new_arr))

 #########################################################################
def insert_exp(node):

    (INSERT, name, position, exp) = node
    assert_match(INSERT, 'insert')
    insert_pos = walk(position)
    insert_val = walk(exp);
    (arr_name, arr) = state.symbol_table.lookup_sym(name)
    ...
    for i,j in enumerate(arr):       
        if j == '[':
            ...
            left_brack_count -=1            
        elif (j == ',' and left_brack_count == 1):
            count+=1
        elif count == insert_pos:
            new_arr = (arr[:i]+ ' ' + str(insert_val) + ',' + arr[i:]) 
            not_found = False
            break
        
    if not_found:
        raise ValueError("Use 'append' or 'prepend' commands to insert at beginning or end of array")
...
def pull_exp(node):
    ...
    #name = name of array to pull value from
    #var is variable to store value into
    
    (PULL, name, position, var) = node
    assert_match(PULL, 'pull')
    
    pull_pos = walk(position)
    (arr_name, arr) = state.symbol_table.lookup_sym(name)
    
    #code to locate position of 'array' element in the string
    #done by spliting the string based on commas
    left_brack_count = 0
    count=0
    not_found = True
    for i,j in enumerate(arr):       
    ...
    state.symbol_table.declare_sym(var, position_value)

 #########################################################################
def empty_arr(node):

    (EMPTY_ARR,) = node
    assert_match(EMPTY_ARR, 'empty_arr')

    return 'nil'

 #########################################################################
def arr_content(node):

    (ARR_CONTENT,content) = node
    assert_match(ARR_CONTENT, 'arr_content')
...
def arr_vals(node):
    ...
    (ARR_VALS, value, content) = node
    assert_match(ARR_VALS, 'arr_vals')
    val = walk(value)
    cont = walk(content)
    
    return str(val) +', ' + str(cont)

 #########################################################################
def arr_exp(node):

    (ARR_EXP, exp) = node
    assert_match(ARR_EXP, 'arr_exp')
